...
//   // Ensure valid parameters before fetching
//   const shouldFetch = from && to && date && seat;

//   const { loading, data } = useFetch(
//     shouldFetch ? `rides/find?from=${from}&to=${to}&seat=${seat}&date=${date}` : null
//   );
...
//   // Initialize state with URL parameters or empty strings
//   const [from, setFrom] = useState(params.get('from') || '');
//   const [to, setTo] = useState(params.get('to') || '');
//   const [date, setDate] = useState(params.get('date') || '');
//   const [seat, setSeat] = useState(params.get('seat') || '');

//   const handleSearch = (e) => {
//     e.preventDefault();
    
//     // Build search query with only valid parameters
//     const searchParams = new URLSearchParams();
//     if (from) searchParams.set('from', from);
//     if (to) searchParams.set('to', to);
//     if (date) searchParams.set('date', date);
//     if (seat) searchParams.set('seat', seat);
...
//   return (
//     <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-2 w-full max-w-4xl">
...
//   // Initially fetch all rides, then fetch filtered rides if search params exist
//   const { loading: allRidesLoading, data: allRidesData } = useFetch('rides');
//   const { loading: searchLoading, data: searchData } = useFetch(
//     from && to ? `rides/find?from=${from}&to=${to}${seat ? `&seat=${seat}` : ''}${date ? `&date=${date}` : ''}` : null
//   );

//   // Determine which data to display
//   const loading = from || to ? searchLoading : allRidesLoading;
//   const initialData = from || to ? searchData : allRidesData;
...
//   useEffect(() => {
//     if (!initialData?.rides) return;
    
//     let rides = [...initialData.rides];
...
//   }, [initialData, sortOption, departureFilters]);
...
//                   </h3>
//                 ) : (
//                   <h3 className="text-lg font-semibold mb-2">All Available Rides</h3>
//                 )}
                
//                 <h3 className="text-md text-muted-foreground mb-4">{filteredRides?.length || 0} rides available</h3>
...
//   // Use the findRides endpoint with empty parameters to get all rides
//   // This avoids the authentication issue with the getAllRides endpoint
//   // const { loading, data } = useFetch(
//   //   from && to 
//   //     ? `rides/find?from=${from}&to=${to}${seat ? `&seat=${seat}` : ''}${date ? `&date=${date}` : ''}`
//   //     // : 'rides/find?from=&to=&seat=1&date=' + new Date().toISOString().split('T')[0]
...
//   // console.log("API Request URL: ", `rides/find?from=${from}&to=${to}${seat ? `&seat=${seat}` : ''}${date ? `&date=${date}` : ''}`);
//   console.log("API Request URL: ", `/rides/find?from=${from}&to=${to}&seat=${seat}&date=${date}`);
...
//         return Object.values(departureFilters).every(value => !value); // If no filters selected, show all
//       });
//     }
...
//                 <h3 className="text-md text-muted-foreground mb-4">{filteredRides?.length || 0} rides available</h3>
//                 {filteredRides?.length === 0 ? (
//                   <h3 className='text-xl font-semibold'>No rides available based on your criteria.</h3>
//                 ) : (
//                   filteredRides?.map((ride) => (
//                     <Link key={ride._id} to={`/ride/${ride._id}`}>
//                       <RideCard details={ride} />
...
//               </DialogTrigger>
//               <DialogContent className="glass-effect bg-[hsl(var(--background),0.95)]">
//                 <Sidebar 
//                   sortOption={sortOption}
//                   onSortChange={handleSortChange}
//                   departureFilters={departureFilters}
...
//                 key={ride._id} 
//                 to={`/ride/${ride._id}`}
//                 className="block transition-all duration-300 hover:translate-y-[-4px]"
//               >
