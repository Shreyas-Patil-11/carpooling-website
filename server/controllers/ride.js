...
export const findRides = async (req, res, next) => {
  try {
    ...
    // if (from) {
    //   query['origin.place'] = new RegExp(from, 'i');
    // }
    
    // if (to) {
    //   query['destination.place'] = new RegExp(to, 'i');
    // }

    if (from && from.trim() !== '') {
      query['origin.place'] = { $regex: from, $options: 'i' };
    }
    
    if (to && to.trim() !== '') {
      query['destination.place'] = { $regex: to, $options: 'i' };
    }
    
    
    if (seat && !isNaN(parseInt(seat))) {
      query['availableSeats'] = { $gte: parseInt(seat) };
    }
